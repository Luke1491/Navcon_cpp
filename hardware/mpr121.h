...
#define MPR_DEVICE_ADDRESS  0x5A  //pull-down to GND
...
#define MPR_R_TCH_THRESHOLD 0x41
#define MPR_R_REL_THRESHOLD 0x42
...
#define MPR_R_ELE_CURR		0x5F
#define MPR_R_ELE_CHARGE_TIME	0x6C //two electrode on each register first bits:0,1,2  second bits:5,6,7; bits 4 and 7 are not used
...
//*************************************************************************
uint8_t MPR_read_reg(uint8_t addr, uint8_t reg); //odczytywanie registru 8-bitowego

uint16_t MPR_read_reg16bit(uint8_t addr, uint8_t reg); //odczytywanie registru 16-bitowego

void MPR_write_reg(uint8_t addr, uint8_t reg, uint8_t data); //wpisywanie danych do wybranego rejestru

void MPR_init(uint8_t addr); //uruchom 4 elektrody w kontrolerze mpr o podanym adresie
